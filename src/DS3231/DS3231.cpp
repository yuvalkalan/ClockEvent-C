...
int writeDS3231(uint8_t *buffer, int bufferLength)
{
    ...
    if (status == PICO_ERROR_GENERIC || status == PICO_ERROR_TIMEOUT)
        return statusToDS3231ErrorCode(status);
    return DS3231_NO_ERROR;
}
...
void bufferToTime(uint8_t *buffer, struct tm *datetime)
{
    ...
    if (tmp & 0x04)
    {
        ...
        if (tmp & 0x02 && datetime->tm_hour != 12)
            datetime->tm_hour += 12;
        // else test if AM and hour is 12
        else if (!(tmp & 0x02) && datetime->tm_hour == 12)
            datetime->tm_hour = 0;
    }
    else
    {
...
int statusToDS3231ErrorCode(int status)
{
    if (status == PICO_ERROR_GENERIC)
        return DS3231_GENERIC_ERROR;
    else if (status == PICO_ERROR_TIMEOUT)
        return DS3231_TIMEOUT_ERROR;
    else
        return DS3231_UNKNOWN_ERROR;
}

// convert DS3231 error code to string
std::string ds3231ErrorString(int status)
{
    if (DS3231_NO_ERROR == status)
        return "No error.";
    if (DS3231_GENERIC_ERROR == status)
        return "Generic error.";
    if (DS3231_TIMEOUT_ERROR == status)
        return "Timeout error.";
    if (DS3231_UNKNOWN_ERROR == status)
